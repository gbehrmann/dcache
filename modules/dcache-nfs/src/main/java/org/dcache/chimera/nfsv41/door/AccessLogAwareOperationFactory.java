...
public class AccessLogAwareOperationFactory extends MDSOperationFactory
{
    ...
    public AccessLogAwareOperationFactory(JdbcFs fs, AccessLogMode accessLogMode)
    {
        checkArgument(accessLogMode != AccessLogMode.NONE);
        _jdbcFs = fs;
    ...
    private class OpSetattr extends OperationSETATTR
    {
        ...
        @Override
        public void process(CompoundContext context,
                            nfs_resop4 result) throws ChimeraNFSException, IOException, OncRpcException
        {
    ...
    private class OpRename extends OperationRENAME
    {
                            ...
                            nfs_resop4 result) throws ChimeraNFSException, IOException, OncRpcException
        {
            ...
            new NetLoggerBuilder(NetLoggerBuilder.Level.INFO, "org.dcache.nfs.rename")
                    ...
                    .add("socket.remote", context.getRemoteSocketAddress().getAddress().getHostAddress())
                    .add("old.path",
                         inode2Path(cSrcParentInode) + "/" + new String(_args.oprename.oldname.value, UTF_8))
                    .add("new.path",
                         inode2Path(cDestParentInode) + "/" + new String(_args.oprename.newname.value, UTF_8))
                    .toLogger(ACCESS_LOGGER);
        }
    }
                            ...
                            nfs_resop4 result) throws ChimeraNFSException, IOException, OncRpcException
        {
            ...
            if (_args.opopen.openhow.opentype == opentype4.OPEN4_CREATE) {
                ...
                new NetLoggerBuilder(NetLoggerBuilder.Level.INFO, "org.dcache.nfs.create")
                        ...
                        .add("socket.remote", context.getRemoteSocketAddress().getAddress().getHostAddress())
                        .add("obj.path",
                             inode2Path(cParentInode) + "/" + new String(_args.opopen.claim.file.value, UTF_8))
    ...
    private class OpRemove extends OperationREMOVE
    {
                            ...
                            nfs_resop4 result) throws ChimeraNFSException, IOException, OncRpcException
        {
            ...
            if (_accessLogMode == AccessLogMode.FULL) {
                nl.add("obj.ino", _jdbcFs.inodeOf(cParentInode, name, FileSystemProvider.StatCacheOption.NO_STAT));
            }
            nl.toLogger(ACCESS_LOGGER);
        }
    }
    ...
    {
        // we get here only if access more MINIMAL or FULL
        if (_accessLogMode == AccessLogMode.MINIMAL) {
            return "inode:" + inode;
        }
